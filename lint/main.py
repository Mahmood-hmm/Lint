...
def print_tools() :
        ...
        Console.print("[yellow] [8][/yellow] yay        [dim][For arch only![/dim]")
        Console.print("[yellow] [9][/yellow] Back")
    ...
    while True :
        ...
        if choice == "1":
            ...
            if manager == "apt" :
                ...
                subprocess.run("sudo dnf config-manager --set-enabled google-chrome", shell=True)
...
def fast_download(app_name) :
    if shutil.which(app_name):
        Console.print(f"[yellow]{app_name} is already exist\n")
    else :
        download(app_name)
...
def tools() :
    while True :
        ...
        if choice == "1" :
            ...
            if is_snapper_exist() :
                Console.print("[yellow]You have snapper installed ![/yellow]\n[cyan]snapper is better if you use btrfs system so keep it ! [/cyan]")
                continuee()
            else :
...
def install_tools() :
    fast_download("git")
    fast_download("fastfetch")
    fast_download("btop")
    fast_download("fish")
    if not is_snapper_exist() :
        fast_download("timeshift")
